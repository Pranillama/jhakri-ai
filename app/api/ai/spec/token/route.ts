...
/**
 * POST /api/ai/spec/token — issue a Trigger.dev public access token scoped to
 * a single spec generation run. Ownership is verified against the `TaskRun`
 * record (the run must belong to the caller) before any token is created, so
 * a token only ever reads a run the user actually started. Used by the client
 * to subscribe to run updates in realtime.
 */
