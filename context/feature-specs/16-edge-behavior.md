...
1. Add connection handles to every node.
   ...
   - give each handle a unique id so a connection records the exact
     side it used and the edge routes to/from that side (not just
     top/bottom)
   ...
   - grow a handle on hover so it is easy to grab and pull from
...
2. Add a default style for new edges.
   ...
   - make the in-progress connection drag preview use the same
     clean right-angle routing as the final edge (no bezier curve)
...
3. Create the custom edge renderer.
   ...
   - let a connected edge be re-pointed by dragging either endpoint
     onto a new node/handle, synced through the collaborative edge flow
...
- Nodes have handles on all four sides, each routing to/from its own side.
- New edges use the custom canvas edge type with arrows.
- The connection drag preview uses right-angle routing.
- Edge hover, selection, and label editing are handled in
  the custom edge renderer.
- Connected edges can be re-pointed by dragging an endpoint.
