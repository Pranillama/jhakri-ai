...
1. Add a pill-shaped control bar at the bottom-left of the canvas.
   ...
   The control buttons are icon-only, so give each one an accessible name with `aria-label` — `title` alone is not a dependable accessible label. Zoom, fit, undo, and redo must stay discoverable to assistive-tech users.
...
- Icon-only control buttons expose an accessible name via `aria-label`.
