...
- **Specs tab Generate button** — the same status-strip component (`RunStatusStrip`) reappears below the Specs tab's "Generate Spec" button while a spec run is in progress, reading the feed's `task: "spec"` message and falling back to "Starting spec generation…" before the first update. The button itself swaps its sparkle icon for a spinner and its label for "Generating…", and disables. The two tabs' status strips key off their own task (`"design"` vs `"spec"`) in the shared `ai-status-feed`, so a run in one tab never locks the other tab's controls.
